...
class CrafterHeuristicAgent:
    """Survival-first heuristic agent for Crafter (v9).

    Key fixes vs v8:
    - Flee ALL monsters within 4 tiles (was 3, only 2+ converging)
    - Sleep safety radius 12 (was 10), no forced sleep at energy=0
    - Flee biased toward water to prevent dehydration
    - Expanded food search range
    """
    ...
    def act(self, obs_image, info):
        ...
        # == P1  TOUCHING MONSTER ==
        if touching:
            isolated = has_sword and len(touching) == 1 and len(close4) <= 1 and health > 4
            if isolated:
        ...
        # == P2.5  FLEE ALL MONSTERS WITHIN 4 TILES ==
        if close4:
        ...
        # == P6  SLEEP (safe radius 12, no forced sleep at energy=0) ==
        far_monsters = self._find_within(sem, H, W, px, py, MONSTERS, 12)
        ...
        # == P12  MAINTENANCE FOOD (expanded range) ==
        if food <= 7:
