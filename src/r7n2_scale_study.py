...
def train_at_scale(
    ...
    trainer.train()
    print('[DEBUG] Training done, deleting trainer...', flush=True)

    # OOM fix: free trainer (optimizer states + grad buffers) before generation eval
    del trainer
    gc.collect()
    torch.cuda.empty_cache()
    print('[DEBUG] Trainer deleted, cache cleared', flush=True)

    model.eval()
    print('[DEBUG] Starting step-level generation eval...', flush=True)

    # Step-level eval (original: same-episode steps may appear in both train/val)
    # OOM fix: batch_size 8->2 to avoid eval-stage OOM on RTX 5090 32GB
    step_eval = generation_eval(model, tokenizer, val_records, batch_size=2)
    print(f'[DEBUG] Step-level eval done: balanced_acc={step_eval["balanced_accuracy"]:.4f}', flush=True)
...
def run_ablation(args):
    ...
    if args.scales:
        ...
        print(f"Filtered scales: {list(scale_points.keys())}")
...
def parse_args():
    ...
    p.add_argument("--scales", nargs="+", default=None,
                   help="Only run these scales (e.g. 100pct 200pct). Default: all")
    return p.parse_args()
