#!/usr/bin/env python3
"""Dose-response curve for retry baseline on death-prone tasks.

Runs retry (no reflection) with varying N trials per episode.
Excludes find-non-living-thing (0% death rate).
"""
...
def run_dose_condition(agent, n_trials, tasks, resolved_tasks, episodes_per_task,
    ...
    for task_short, task_resolved in zip(tasks, resolved_tasks):
        print(f"\n{'='*60}")
        print(f"[N={n_trials}] Task: {task_short}")
        print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Retry dose-response curve on death-prone tasks")
    parser.add_argument("--n-values", type=str, default="1,5,10",
                        help="Comma-separated retry N values (default: 1,5,10)")
    ...
    parser.add_argument("--output-dir", type=str,
                        default="results/retry_dose_response/")
    ...
    print(f"Dose-response plan: N={n_values}, {len(tasks)} tasks, "
    ...
    for n in n_values:
        print(f"\n{'#'*60}")
        print(f"# CONDITION: retry N={n}")
        print(f"{'#'*60}")

        cond_dir = os.path.join(args.output_dir, f"retry_n{n}")
        ...
        print(f"\n[N={n}] Aggregate: DR={stats['aggregate']['death_rate']:.1%}")
    ...
    print(f"\nReference: N=3 baseline DR=40.9% (220 eps)")
